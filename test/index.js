...
describe('API ENDPOINT TESTING', () => {
  it('GET Landing Page', (done) => {
    chai.request(app).get('/api/v1/member/landingpage').end((err, res) => {
      expect(err).to.be.null
      expect(res).to.have.status(200)
      expect(res.body).to.be.an('object')
      expect(res.body).to.have.property('hero')
      expect(res.body.hero).to.have.all.keys('travelers', 'tresure', 'cities')
  ...
  it('GET Detail Page', (done) => {
    chai.request(app).get('/api/v1/member/detailpage/5e96cbe292b97300fc902223').end((err, res) => {
      expect(err).to.be.null
      expect(res).to.have.status(200)
      expect(res.body).to.be.an('object')
  ...
  it('POST Booking Page', (done) => {
    ...
    chai.request(app).post('/api/v1/member/bookingpage')
